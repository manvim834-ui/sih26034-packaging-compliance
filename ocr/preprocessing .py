...
def image_quality(image):
    ...
    return {
        ...
        "glare_ratio": round(float(np.mean(gray >= 245)), 4)
    }


def reduce_glare(color_img, gray_img):
    """
    Conservative glare/reflection reduction for glossy packaging.

    Detects specular highlights (glare) rather than plain white ink or
    background: a glare spot is both very bright AND low-saturation in HSV,
    which distinguishes it from ordinary white text/labels that are bright
    but not desaturated in the same way. Only those small detected regions
    are inpainted - the rest of the image is untouched, so this can't
    blur or destroy text elsewhere on the label.

    Same safe-fallback pattern as deskew()/correct_perspective() in this
    file: if no glare is detected, the grayscale input is returned
    unchanged rather than forcing a transformation.
    """
    hsv = cv2.cvtColor(color_img, cv2.COLOR_BGR2HSV)
    value = hsv[:, :, 2]
    saturation = hsv[:, :, 1]

    glare_mask = ((value > 240) & (saturation < 60)).astype(np.uint8) * 255
    ...
    if not np.any(glare_mask):
        return gray_img  # no glare detected, safe fallback

    return cv2.inpaint(gray_img, glare_mask, 3, cv2.INPAINT_TELEA)
...
def generate_variants(
    ...
    builders = {
        ...
        "glare_reduced": lambda: reduce_glare(original, gray),
    }
